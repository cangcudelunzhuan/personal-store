...
function paramType(parameter, rules, Tip) {
  ...
  const checkResult = fields.every((item) => {
    ...
    const result = currentFieldsRules.every((rItem) => {
      // 首字母小写
      rItem = `${rItem}`.replace(/./, (a) => { return a.toLowerCase(); });
  ...
  return {
    ...
    rule(ruleName, rule, message) {
      ...
      if (!Common.isType(rule, 'function')) {
        const itemRule = Common.seek()
          .equal(toLower(ruleName) === 'string', String)
          .equal(toLower(ruleName) === 'number', Number)
          .equal(toLower(ruleName) === 'object', Object)
          .equal(toLower(ruleName) === 'function', Function)
          .equal(toLower(ruleName) === 'array', Array)
          .equal(toLower(ruleName) === 'date', Date)
          .equal(toLower(ruleName) === 'boolean', Boolean)
          .get();
        if (itemRule) {
          message = rule;
          rule = itemRule;
